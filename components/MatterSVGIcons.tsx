...
const MatterSvgIcons = ({ header }: MatterProps) => {
  ...
  useEffect(() => {
    ...
    const createIcon = (svg: string, y: number) => {
      ...
      const icon = Bodies.rectangle(randomX, y, iconSize, iconSize, {
        ...
        plugin: {
          wrap: {
            min: { x: 10, y: -Infinity },
            max: { x: viewportWidth - 10, y: Infinity }
          }
        }
      });
    ...
    const illustratorSvg = `
      <svg xmlns="http://www.w3.org/2000/svg" fill="#330000" stroke="#FF9A00" stroke-width="0" viewBox="0 0 24 24">
    ...
    `;
    const reactIcon = createIcon(reactSvg, 500);
    ...
    if (!header) {
      const vsCodeSVG =`
        <svg xmlns="http://www.w3.org/2000/svg" fill="#3992E0" stroke="currentColor" stroke-width="0" viewBox="0 0 16 16">
      ...
      `;
      const vsCodeIcon = createIcon(vsCodeSVG, 500);
      const psSVG =`
        <svg stroke="currentColor" fill="#2B4687" stroke-width="0" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      `;
      const psIcon = createIcon(psSVG, 500);
